...
def set_bounds(t,b):
    global THE_SOLVER
    THE_SOLVER.add_assertion(Equals(MAX_TIME, Real(t)))
    THE_SOLVER.add_assertion(Equals(MAX_BAND, Real(b)))
    THE_SOLVER.add_assertion(Equals(MIN_TIME, Real(0)))
    THE_SOLVER.add_assertion(Equals(MIN_BAND, Real(0)))
...
def add_req(reqName, reqTime, reqBandwidth, currSolver):
    ...
    xo = Symbol(reqName+'_Xo', REAL)
    yo = Symbol(reqName+'_Yo', REAL)
    xe = Symbol(reqName+'_Xe', REAL)
    ye = Symbol(reqName+'_Ye', REAL)
    # Request dimensions
    currSolver.add_assertion(Equals(Minus(xe,xo),Real(reqTime)))
    currSolver.add_assertion(Equals(Minus(ye,yo),Real(reqBandwidth)))
...
if len(reqInput) <= 1:
    ...
    MAX_TIME = Symbol("MAX_TIME", REAL) #Width
    MAX_BAND = Symbol("MAX_BAND", REAL) #Height
    MIN_TIME = Symbol("MIN_TIME", REAL)
    MIN_BAND = Symbol("MIN_BAND", REAL)
